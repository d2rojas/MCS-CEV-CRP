...
echo "🧪 Testing with ultra-simple Node.js server (Alpine Linux)"
...
# Build and push ultra-simple image
echo "Step 2: Building ultra-simple image (Alpine Linux)..."
docker build -f Dockerfile.ultra-simple -t $ECR_REPOSITORY .
docker tag $ECR_REPOSITORY:latest $AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/$ECR_REPOSITORY:latest

echo "Step 3: Pushing ultra-simple image..."
...
echo "⏳ Wait 2-3 minutes and check if health check passes"
echo "💡 If this works, the problem is with our complex container"
echo "💡 If this fails, the problem is with App Runner configuration"
